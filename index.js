...
function search_product(){
    ...
    let list_product_search = '',list = new Array();
    for (let i = 0; i < list_item.length; i++) {
        if ((list_item[i].watch_brand.toLowerCase()).includes(product_search) && product_search != '') {
            list.push(list_item[i]);
    ...
    if (list_product_search == '') {
        ...
        let so_luong_trang = Math.ceil(list.length / 6);
        let html = '';
        for (let i = 1; i <= so_luong_trang; i++) {
            html += `<button onclick="show_product_depend_on_page(${i},${list})"><div class="page">${i}</div></button>`
        }
        document.getElementById('right').innerHTML = list_product_search;
        document.getElementById('show-page').innerHTML = html;
        // show_product_depend_on_page()
    }
}
...
function show_page() {
    ...
    let html = '';
    for (let i = 1; i <= so_luong_trang; i++){
        html += `<button onclick="show_product_depend_on_page(${i},${list_item})"><div class="page">${i}</div></button>`
    }
    document.getElementById('show-page').innerHTML = html;
}


function show_product_depend_on_page(page_current,list_item) {
    // list_item = JSON.parse(localStorage.getItem('list_product'));
    console.log(list_item);
    let html = '';
